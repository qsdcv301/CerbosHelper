...
- 기본 SQL scope alias는 `cb`다. custom injector가 alias를 제공하지 않는 경우에는 top-level `FROM` / `JOIN`에서 resource alias를 fallback으로 감지한다.
...
| `CerbosSqlPredicateInjector` | `DefaultCerbosSqlPredicateInjector` | 기본 `SELECT cb.* FROM (...) cb WHERE (...)`와 다른 SQL shape 처리 |
